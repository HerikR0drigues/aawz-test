        ...
        <div className="md:w-3/5 w-full">
            <div className="space-y-6 p-6 bg-aawzBlack rounded-2xl shadow-lg border-2 border-aawzMain flex flex-col min-h-[640px] max-h-[640px]">
                ...
                <div className="space-y-2 overflow-y-auto scrollbar-thumb-aawzMain scrollbar-track-aawzBlack scrollbar-thin px-2">
                    {usuarios && usuarios.length > 0 ? (  // Verifique se 'usuarios' existe
                        usuarios.map((usuario, index) => (
                            <div
                                key={index}
                                onClick={() => toggleExpand(index)}
                                className={`bg-zinc-900 px-4 py-2 rounded-md shadow-md cursor-pointer transition-all duration-300 border-2 border-gray-300 hover:border-aawzMain hover:bg-zinc-800 ${expandedIndex === index ? 'max-h-full' : 'max-h-20'
                                    }`}
                            >
